...
class TestKLV(unittest.TestCase):
    ...
    def test_decode_ber(self):
        ...
        self.assertEqual(klv.decode_ber(
            [0x83, 0x01, 0x01, 0x00])[0], 65536 + 256)
        self.assertEqual(klv.decode_ber(
            [0x83, 0x01, 0x01, 0x01])[0], 65536 + 257)

    def test_decode_ber_with_strings(self):
        self.assertEqual(klv.decode_ber(b'\x7f'), (127, 1,))
        self.assertEqual(klv.decode_ber(b'\x81\xff'), (255, 2,))
        self.assertEqual(klv.decode_ber(b'\x82\x01\x00'), (256, 3,))
        self.assertEqual(klv.decode_ber(b'\x83\x00\x01\x00'), (256, 4,))
        self.assertEqual(klv.decode_ber(
            b'\x83\x01\x01\x01'), (65536 + 257, 4,))

    def test_encode_ber(self):
        self.assertEqual(klv.encode_ber(0, 4),
                         bytearray([0x83, 0x00, 0x00, 0x00]))
        self.assertEqual(klv.encode_ber(10, 4),
                         bytearray([0x83, 0x00, 0x00, 0x0a]))
        self.assertEqual(klv.encode_ber(127, 4),
                         bytearray([0x83, 0x00, 0x00, 0x7f]))
        self.assertEqual(klv.encode_ber(255, 4),
                         bytearray([0x83, 0x00, 0x00, 0xff]))
        self.assertEqual(klv.encode_ber(256, 4),
                         bytearray([0x83, 0x00, 0x01, 0x00]))
        self.assertEqual(klv.encode_ber(65535, 4),
                         bytearray([0x83, 0x00, 0xff, 0xff]))
        self.assertEqual(klv.encode_ber(65536, 4),
                         bytearray([0x83, 0x01, 0x00, 0x00]))
        self.assertEqual(klv.encode_ber(65546, 4),
                         bytearray([0x83, 0x01, 0x00, 0x0a]))
        self.assertEqual(klv.encode_ber(65536 + 256, 4),
                         bytearray([0x83, 0x01, 0x01, 0x00]))
        self.assertEqual(klv.encode_ber(65536 + 257, 4),
                         bytearray([0x83, 0x01, 0x01, 0x01]))

    def test_encode_ber_with_strings(self):
        self.assertEqual(klv.encode_ber(0, 4), bytearray(b'\x83\x00\x00\x00'))
        self.assertEqual(klv.encode_ber(10, 4), bytearray(b'\x83\x00\x00\x0a'))
        self.assertEqual(klv.encode_ber(127, 4),
                         bytearray(b'\x83\x00\x00\x7f'))
        self.assertEqual(klv.encode_ber(65536, 4),
                         bytearray(b'\x83\x01\x00\x00'))
        self.assertEqual(klv.encode_ber(65546, 4),
                         bytearray(b'\x83\x01\x00\x0a'))
        self.assertEqual(klv.encode_ber(65536 + 257, 4),
                         bytearray(b'\x83\x01\x01\x01'))
    ...
    def test_encode_klv_key_length_18(self):
        ...
        value = list(
            'these are little endian encoded Unicode characters'.encode('utf-16le'))
        k = klv.encode(key, value)
    ...
    def test_decode_klv(self):
        ...
        self.assertEqual(klv.decode(k, 16),
                         (bytearray(key), bytearray(value),))
    ...
    def test_encode_decode_klv(self):
        ...
        value = list(
            'these are little endian encoded Unicode characters'.encode('utf-16le'))
        k = klv.encode(key, value)
        self.assertEqual(klv.decode(k, 18),
                         (bytearray(key), bytearray(value),))
